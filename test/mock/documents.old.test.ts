...
// https://github.com/typesense/typesense-js/blob/master/test/Typesense/Documents.spec.js
// https://github.com/typesense/typesense-js/blob/master/test/Typesense/Document.spec.js
...
mod.describe("documents", () => {
  ...
  mod.describe(".retrieve", () => {
    mod.it("retrieves a document", async () => {
      fm.get(
        new URL("/collections/companies/documents/124", url).toString(),
        new Response(JSON.stringify(documentResult), {
          status: 200,
          headers: {
      ...
      const returnData = await document.retrieve();

      asserts.assertEquals(returnData, documentResult);
    });
  });

  mod.describe(".update", () => {
    mod.it("updates a document", async () => {
      ...
      fm.patch(
        ...
        (url) => {
          asserts.assertEquals(
            new URL(url).searchParams.get("dirty_values"),
            "coerce_or_reject",
          );
          return new Response(JSON.stringify(partialDocument), {
            status: 200,
            headers: {
              "Content-Type": "application/json",
            },
      ...
      const returnData = await document.update(partialDocument, {
        dirty_values: "coerce_or_reject",
      });

      asserts.assertEquals(returnData, partialDocument);
    });
  });

  mod.describe(".delete", () => {
    mod.it("deletes a document", async () => {
      fm.delete(
        new URL("/collections/companies/documents/124", url).toString(),
        new Response(JSON.stringify(documentResult), {
          status: 200,
      ...
      const returnData = await document.delete();
  ...
  const apiKey = "abcd";
  let typesense: TypesenseClient;
  let documents: Documents<any>;
  let document: Document<any>;
  let anotherDocument: Document<any>;
  let apiCall: ApiCall;
  ...
  mod.describe(".search", () => {
    mod.it("searches the documents in a collection", async () => {
      ...
      let returnData = await documents.search(searchParameters);

      expect(returnData).to.eventually.deep.equal(stubbedSearchResult).notify(
        done,
      );
    });
    mod.it("searches with and without cache", async function () {
      let searchParameters = [
        {
          q: "Stark",
          query_by: "company_name",
    ...
    it("upserts the document", function (done) {
      mockAxios
        ...
        .reply((config) => {
          ...
          return [201, JSON.stringify(document), {
            "content-type": "application/json",
          }];
        });
      ...
      it("passes the query parameter to the API", function (done) {
        mockAxios
          ...
          .reply((config) => {
            ...
            return [200, JSON.stringify({ success: true }), {
              "content-type": "text/plain",
            }];
          });
  ...
  describe(".import", function () {
    context("when a query paramater is passed", function () {
      it("passes the query parameter to the API", function (done) {
        ...
        let jsonlData = [document, anotherDocument].map((document) =>
          JSON.stringify(document)
        ).join("\n");
    ...
    context("when an array of docs is passed", function () {
      it(
        "converts it to JSONL and returns an array of results",
        function (done) {
          mockAxios
            .onPost(
              apiCall.uriFor(
