// import { asserts, fm, mod } from "../../dev_deps.ts";
// import { SearchClient as TypesenseSearchClient } from "../../src/mod.ts";
//
// // https://github.com/typesense/typesense-js/blob/master/test/Typesense/SearchClient.spec.js
//
// mod.describe("SearchClient", () => {
//   const url = new URL("http://node0:8108");
//   const apiKey = "abcd";
//   let typesense: TypesenseSearchClient;
//
//   mod.beforeEach(() => {
//     typesense = new TypesenseSearchClient({
//       nodes: [url],
//       apiKey,
//       randomizeNodes: false,
//       cacheSearchResultsForSeconds: 2 * 60,
//     });
//   });
//   mod.it("should set the right default configuration values", () => {
//     asserts.assertEquals(typesense.configuration.nodes, [url]);
//     // asserts.assertEquals(typesense.configuration.connectionTimeoutSeconds, 5);
//     asserts.assertEquals(typesense.configuration.apiKey, "abcd");
//   });
//   mod.it("should only expose the search endpoints", () => {
//     asserts.assert(
//       typeof typesense.collections("xyz").documents().search === "function",
//     );
//     asserts.assert(typeof typesense.multiSearch.perform === "function");
//   });
//
//   mod.it(
//     "should send the api key via headers, when key is over 2000 characters, even if sendApiKeyAsQueryParam is true",
//     async () => {
//       const configuration = {
//         nodes: [url],
//         apiKey:
//           "",
//         sendApiKeyAsQueryParam: true,
//       };
//       typesense = new TypesenseSearchClient(configuration);
//       const searches = {
//         searches: [{ q: "term1", query_by: "" }, { q: "term2", query_by: "" }],
//       };
//       const commonParams = {
//         collection: "docs",
//         query_by: "field",
//       };
//
//       fm.post(
//         `begin:${new URL("/multi_search", url).toString()}`,
//         (url) => {
//           asserts.assertEquals(
//             new URL(url).searchParams.get("x-typesense-api-key"),
//             null,
//           );
//           return new Response("{}", {
//             status: 200,
//             headers: {
//               "Content-Type": "application/json",
//             },
//           });
//         },
//         {
//           body: searches,
//         },
//       );
//
//       const returnData = await typesense.multiSearch.perform(
//         searches,
//         commonParams,
//       );
//
//       asserts.assertEquals(returnData, {});
//     },
//   );
// });
